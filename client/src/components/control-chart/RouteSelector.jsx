 ...
 * (e.g. West Line MAS-JTJ, North Line MAS-GDR, Main Line JTJ-SA-ED-CBE)
 * comprising constituent block sections.
 * Step 03 in the canonical controller hierarchy.
 */
export default function RouteSelector({
  ...
  const isSelected = Boolean(selectedRouteId);
  ...
  return (
    <div className={`flex flex-col space-y-1.5 p-2 rounded-lg border transition-all ${
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="route-selector" className="flex items-center space-x-1.5 text-[11px] font-bold tracking-wider uppercase cursor-pointer">
          ...
          <span className={isSelected ? 'text-cyan-300' : isDivisionChosen ? 'text-slate-300' : 'text-slate-500'}>
            Route / Corridor
          </span>
        </label>
        {isSelected && (
      ...
      <div className="relative">
        ...
        >
          {!isDivisionChosen ? (
            ...
            <option value="" className="bg-slate-900 text-slate-400">-- Standard Division Route --</option>
          ) : (
            <>
              <option value="" className="bg-slate-900 text-slate-400">-- Select Route / Corridor --</option>
              {routes.map((route) => {
                const rId = route.id || route.routeName || route.routeCode;
                return (
                  <option key={`route-${rId}`} value={rId} className="bg-slate-900 text-slate-100">
